...
export const singUpFx = createEffect(async (payload: ISignUpFx) => {
  console.log('singUpFx called', payload)
  const { name, email, password, isOAuth } = payload

  console.log('singUpFx called', { name, email, password, isOAuth })

  if (isOAuth) {
    await oauthFx({ name, email, password })
    return
  }
